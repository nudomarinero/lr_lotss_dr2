...
default_combined_name = os.path.join(data_path, "samples", "test_combined.fits")
default_params = pickle.load(
    open(os.path.join(BASEPATH, "lofar_params.pckl"), "rb"))
colour_limits = np.array([0.7, 1.2, 1.5, 2. , 2.4, 2.8, 3.1, 3.6, 4.1])
default_threshold = 0.404
max_major = 15
radius = 15
...
def apply_lr(
    input_catalogue, 
    output_catalogue,
    base_optical_catalogue=default_combined_name, 
    params=default_params,
    threshold=default_threshold,
    n_cpus=None
    ):
    """Apply the LR method to an input catalogue matching it to the 
    base optical catalogue.
    """
    bin_list, centers, Q_0_colour, n_m, q_m = params
    ## Load the catalogues
    print("Load optical catalogue")
    combined = Table.read(default_combined_name)
    ...
    if (n_cpus is None) or (n_cpus > (n_cpus_total-1)):
    ...
    print("Save output")
    columns_save = combined.colnames + lofar.colnames + [
        'colour', 'category', 'lr', 'lr_dist']
    pwl[columns_save].filled().write(output_catalogue, format="fits")

if __name__ == "__main__":
    # Name of input catalogue
    # Name of matching optical catalogue (with default)
    # Name of parameters file (with default)
    ...
    apply_lr(input_catalogue, output_catalogue)
